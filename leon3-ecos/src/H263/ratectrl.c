...

{
  ...
  if (pict_type == PCT_INTER) {
    ...
    printf("stderr:No need to call InititializeQuantizer() for Intra picture\n");
    exit(-1);
  }
  else  {
    printf("stderr:Error (InitializePictureRate): picture type unkown.\n");
    exit(-1);
  }  
#if 1
...

{
  ...
  /* the update equation for newQP in TMN4 document section 3.7 */
    ...
    printf("stderr:Error (UpdateQuantizer): picture type unkown.\n");
  }
