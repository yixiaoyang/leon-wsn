...
int8u_t OV7660_init(void) {
	...
	for (i = 0; i < 1; i++) {
		...
		if (FAIL == read_OV7660_reg(0x0a, &pid)) {
			dprintf("REAd 0x0A error!\n");
		} else {
			dprintf("REG[OA](76)=%2x\n", pid);
		}
	}
	dprintf("-----------------------------------\n");
	for (i = 0; i < 1; i++) {
		...
		if (FAIL == read_OV7660_reg(0x0b, &ver)) {
			...
			dprintf("REG[0B](73)=%2x\n", ver);
		}
	}

	/*regs init*/
	dprintf("start write regs\n");
