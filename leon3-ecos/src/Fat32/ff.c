...
void fs_test() {
	...
	char Buff[512] __attribute__ ((aligned (1))) ;		/* Working buffer */
	...
	printf(
			...
			(fs->fs_type == FS_FAT12) ? "FAT12"
					: (fs->fs_type == FS_FAT16) ? "FAT16" : "FAT32",
	...
	ptr2 = dfn;
	printf("Opening %s\n", ptr);
	rc = f_open(&File1, ptr, FA_OPEN_EXISTING | FA_READ);
	printf('\n');
	if (rc) {
		put_rc(rc);
		return;
	...
	printf("Creating %s\n", ptr2);
	rc = f_open(&File2, ptr2, FA_CREATE_ALWAYS | FA_WRITE);
	if (rc) {
		put_rc(rc);
		f_close(&File1);
		return;
	...
	for (;;) {
		printf(".");
		rc = f_read(&File1, Buff, blen, &s1);

		//for(i = 0; i < 64; i++)
			//printf("%2x ",(char)Buff[i]);

		if (rc || s1 == 0) {
			put_rc(rc);
			printf("File1 reaches EOF\n");
			break;
			//return ;  //error or eof
		}
		rc = f_write(&File2, Buff, s1, &s2);
		p1 += s2;
		if (rc || s2 < s1) {
			printf("disk full\n");
			break;
