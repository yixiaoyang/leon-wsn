    ...
    for j=1:h
        ...
        elseif(bitxor(i0,1)&&bitxor(j0,1))
             newimg(i,j,3)=Bayer(i,j);  
                     if((i>1&& j>1)&&(i<w&&j<h))
                         newimg(i,j,2)=(Bayer(i,j-1)+Bayer(i-1,j))/2;%写错了
        ...
        elseif(bitxor(i0,1)&&bitxor(j0,1))
             newimg2(i,j,3)=Bayer(i,j);  
                     if((i>2&& j>2)&&(i<w-1&&j<h-1))
                         ...
                         newimg2(i,j,2)=(Bayer(i,j-1)+Bayer(i-1,j))/2;
    ...
    for j=1:h
        ...
        if (bitand(i0,1) && bitand(j0,1))
            ...
            if((i>1&& j>1)&&(i<w&&j<h))
            	...
            	newimg4(i,j,3)=(Bayer(i+1,j+1) + Bayer(i-1,j-1) + Bayer(i-1,j+1) + Bayer(i+1,j-1))/4;	%Blue分量
            end
        
        %% Process Green Part,有两种分布方式
        elseif ( (bitand(i0,1) && bitxor(j0,1) )||(bitxor(i0,1)&&bitand(j0,1)))
            newimg4(i,j,2)=Bayer(i,j);
            if( bitand(i0,1) && bitxor(j0,1) )
                if((i>1&& j>1)&&(i<w&&j<h))
                    newimg4(i,j,1)=(Bayer(i,j-1) + Bayer(i,j+1))/2; %Red
                    newimg4(i,j,3)=(Bayer(i+1,j) + Bayer(i-1,j))/2; %Blue
                end
            end
            if (bitxor(i0,1)&&bitand(j0,1))
                if((i>1&& j>1)&&(i<w&&j<h))
                    newimg4(i,j,1)=(Bayer(i-1,j) + Bayer(i+1,j))/2; %Red
                    newimg4(i,j,3)=(Bayer(i,j+1) + Bayer(i,j-1))/2; %Blue
                end
            end
        ...
        elseif(bitxor(i0,1)&&bitxor(j0,1))
             ...
             if((i>1&& j>1)&&(i<w&&j<h))
                 newimg4(i,j,2)=(Bayer(i+1,j) + Bayer(i,j+1) + Bayer(i-1,j) + Bayer(i,j-1))/4;
                 newimg4(i,j,1)=(Bayer(i+1,j+1) + Bayer(i-1,j-1) + Bayer(i-1,j+1) + Bayer(i+1,j-1))/4;%Red
             end
        end
    end
end
        ...
        if (bitand(i0,1) && bitand(j0,1))
            ...
            if((i>1&& j>1)&&(i<w&&j<h))
            	if(icompare( Bayer(i,j-1),Bayer(i,j+1), Bayer(i+1,j),Bayer(i-1,j) ) == 1)
            		newimg5(i,j,2)=( Bayer(i+1,j)+Bayer(i-1,j) ) / 2;
            	elseif(icompare( Bayer(i,j-1),Bayer(i,j+1), Bayer(i+1,j),Bayer(i-1,j) ) == -1)
            		newimg5(i,j,2)=( Bayer(i,j+1) + Bayer(i,j-1) ) /2; 	%Green分量
            	else 
...
for i=1:w
    for j=1:h
        i0=bitget(i,1);j0=bitget(j,1);
        %% Process Red Part
        if (bitand(i0,1) && bitand(j0,1))	%red
            if((i>1&& j>1)&&(i<w&&j<h))
            	newimg5(i,j,3)=(  Bayer(i-1,j-1) + Bayer(i-1,j) + Bayer(i-1,j+1) + Bayer(i,j-1) + Bayer(i,j+1) + Bayer(i+1,j-1) + Bayer(i+1,j) + Bayer(i+1,j+1) ) / 4 - newimg5(i,j,1);	%Blue
            end 
        elseif(bitxor(i0,1)&&bitxor(j0,1))	%blue
    		if((i>1&& j>1)&&(i<w&&j<h))
    			newimg5(i,j,1)=(  Bayer(i-1,j-1) + Bayer(i-1,j) + Bayer(i-1,j+1) +  Bayer(i,j-1) + Bayer(i,j+1) +  Bayer(i+1,j-1) + Bayer(i+1,j) + Bayer(i+1,j+1) ) / 4 - newimg5(i,j,3);	%Red
            end
    	end
	end
end
...
%---------------------------------------------------------------------------------------
%% Demosaic，block size:2*2
newimg6=zeros(w,h,3,'uint8');
for i=1:w
    for j=1:h
        i0=bitget(i,1);j0=bitget(j,1);
        %% Process Red Part
        if (bitand(i0,1) && bitand(j0,1))
            newimg6(i,j,1)=Bayer(i,j);
            if((i>1&& j>1)&&(i<w&&j<h))
            	newimg6(i,j,2)=(Bayer(i+1,j)+Bayer(i,j+1))/2; 	%Green分量
            	newimg6(i,j,3)=Bayer(i+1,j+1);	%Blue分量
            end
        
        %% Process Green Part
        elseif ( (bitand(i0,1) && bitxor(j0,1) )||(bitxor(i0,1)&&bitand(j0,1)))
            
            newimg6(i,j,2)=Bayer(i,j);
            if( bitand(i0,1) && bitxor(j0,1) )
                if((i>1&& j>1)&&(i<w&&j<h))
                    newimg6(i,j,1)=Bayer(i,j+1); %% R
                    newimg6(i,j,3)=Bayer(i+1,j); %% B
                end
            end
            if (bitxor(i0,1)&&bitand(j0,1))
                if((i>1&& j>1)&&(i<w&&j<h))
                    newimg6(i,j,1)=Bayer(i+1,j);
                    newimg6(i,j,3)=Bayer(i,j+1);
                end
            end
       
        %% Process Blue Part
        elseif(bitxor(i0,1)&&bitxor(j0,1))
             newimg6(i,j,3)=Bayer(i,j);  
                 if((i>1&& j>1)&&(i<w&&j<h))
                     newimg6(i,j,2)=(Bayer(i,j+1)+Bayer(i+1,j))/2;
                     newimg6(i,j,1)=Bayer(i+1,j+1);
                 end
        end
    end
end
