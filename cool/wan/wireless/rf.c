...
/*
	...
	i=0; while(--i);
// */
	delayus(100);  // Delay after reset
	uz2400d_sw(UZS_RFCTL, 0x00);
//	uz2400d_sw(UZS_SLPACK, 0x7F);

//	while(uz2400d_lr(UZL_RFSTATE)!=0x50);

	/* Set network parameters, addresses defined in rf.h */
...
/********************************************************************
 Function: Set transmission speed to 250Kbps/1M/2M.
 [
 	> None
	< None
 ]
...
int8u_t RF_Rx(int8u_t *buffer)
{
	...
	/* Read length field of RXFIFO */
	len = uz2400d_lr(0x300); 		
	/* Restore received packet */
	uz2400d_lr_block(0x301, buffer, len); 
	/* RXFIFO flashed when the length field and the last byte of the \
	  frame are read or trigger a RX flash at SREG0x0D[0]. */
