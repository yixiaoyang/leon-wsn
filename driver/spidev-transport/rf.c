 ...
 */
u32 RF_CAL_SLEEP_CLK(struct spidev_data* rf_dev,u8 clk_div)
{
 ...
 *]
 */
void RF_AUTO_EXT_PA_CTRL_ON(struct spidev_data* rf_dev)
{
	uz2400d_lw(rf_dev,0x22f, (uz2400d_lr(rf_dev,0x22f)&~0x7)|0x1);
}

void RF_AUTO_EXT_PA_CTRL_OFF(struct spidev_data* rf_dev)
{
 ...
 *]
 */
void RF_NORMAL_SPEED(struct spidev_data* rf_dev)
{
 ...
 *]
 */
void RF_TURBO_SPEED_1M(struct spidev_data* rf_dev)
{
 ...
 *]
 */
void RF_TURBO_SPEED_2M(struct spidev_data* rf_dev)
{
	uz2400d_lw(rf_dev,0x206, 0x70); 			//RF optimized control
	uz2400d_lw(rf_dev,0x207, uz2400d_lr(rf_dev,0x207)&~0x10); 	//RF optimized control
 ...
 *]
 */
void RF_SET_PAN_ID(struct spidev_data* rf_dev,u16 panid)
{
 ...
 *]
 */
void RF_SET_SHORT_ADDRESS(struct spidev_data* rf_dev,u16 saddr)
{
 ...
 *]
 */
void RF_SET_MAC_ADDR(struct spidev_data* rf_dev,const u8* eadr)
{
...
void RF_CHIP_INITIALIZE(struct spidev_data* rf_dev)
{
	...
	printk("UZS_GATECLK:%02x,%02x(0x88)\n",
		(uz2400d_sr(rf_dev,UZS_GATECLK)),
		uz2400d_sr(rf_dev,UZS_FIFOEN));/*UZS_FIFOEN:0x88*/
	...
	printk("LRF020 set Address: ");
	printk("%2x: %2x: %2x: %2x: %2x: %2x: %2x: %2x: ",EADR_0,EADR_1,EADR_2,EADR_3,EADR_4,EADR_5,EADR_6,EADR_7);
	printk("\n");
...
void RF_TEST(struct spidev_data* rf_dev)
{
	...
	uz2400d_sw(rf_dev,UZS_SOFTRST, 0x07);			/* Reset register */
	for(i = 0; i < 32; i++){
		printk("UZS_GATECLK:%02x(0x88)\n",uz2400d_sr(rf_dev,UZS_FIFOEN));
	}
}
